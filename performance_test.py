...
class PerformanceTest:
    ...
    async def test_stream_creation_performance(self, num_streams: int = 10) -> Dict:
        ...
        logger.info(f"Testing stream creation performance with {num_streams} streams")
        ...
        for i in range(num_streams):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to create stream {i}: {response.status_code}")
    ...
    async def test_concurrent_clients_same_stream(self, num_clients: int = 5) -> Dict:
        ...
        logger.info(f"Testing concurrent clients on same stream with {num_clients} clients")
        ...
        response = await self.client.post(f"{self.proxy_url}/streams", json=payload)
        if response.status_code != 200:
            raise Exception(f"Failed to create test stream: {response.status_code}")
    ...
    async def test_channel_zapping_cleanup(self, num_zaps: int = 10) -> Dict:
        ...
        logger.info(f"Testing channel zapping cleanup with {num_zaps} channel changes")
    ...
    async def test_failover_performance(self) -> Dict:
        ...
        payload = {
            ...
            "failover_urls": ["https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8"],  # This should work
        ...
        result = {
            ...
            "transparent_failover": access_success  # If successful, failover was transparent
        }
...
async def main():
    parser = argparse.ArgumentParser(description='m3u-proxy Performance Testing')
    parser.add_argument('--proxy-url', default='http://localhost:8001', help='Proxy URL')
