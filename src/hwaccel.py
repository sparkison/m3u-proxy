...
class HardwareAccelDetector:
    ...
    def _load_config(self) -> HardwareAccelConfig:
        ...
        available = env_vars.get('HW_ACCEL_AVAILABLE', os.getenv('HW_ACCEL_AVAILABLE', 'false')).lower() == 'true'
    ...
    def log_capabilities(self):
        ...
        if self.config.available:
            logger.info(f"🚀 Hardware acceleration ENABLED: {self.config.type} ({self.config.device})")
