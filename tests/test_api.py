...
class TestAPI:
    ...
    def test_create_stream_get_deprecated(self, client, mock_stream_manager):
        response = client.get("/create_stream?url=http://example.com/stream.m3u8")
        assert response.status_code == 200
    ...
    def test_get_stream_info_exists(self, client, mock_stream_manager):
        ...
        data = response.json()
        assert data["stream_id"] == "test_stream_123"
        assert data["original_url"] == "http://example.com/test.m3u8"
    ...
    def test_playlist_endpoint(self, client, mock_stream_manager):
        mock_stream_manager.proxy_playlist = AsyncMock(return_value="#EXTM3U\nsegment1.ts")
        
        response = client.get("/playlist/test_stream_123")
        assert response.status_code == 200
    ...
    def test_proxy_endpoint_segment(self, client, mock_stream_manager):
        async def mock_proxy_generator():
            yield b"segment_data_chunk_1"
            yield b"segment_data_chunk_2"
        
        mock_stream_manager.proxy_segment = AsyncMock(return_value=mock_proxy_generator())
        
        response = client.get("/proxy/test_stream_123/segment1.ts")
        assert response.status_code == 200
    ...
    def test_direct_stream_endpoint(self, client, mock_stream_manager):
        ...
        mock_stream_manager.proxy_direct_stream = AsyncMock(return_value=mock_stream_generator())
        
        response = client.get("/direct/test_stream_123")
        assert response.status_code == 200
    ...
    def test_error_handling_stream_creation_failure(self, mock_sm, client):
        mock_sm.get_or_create_stream = AsyncMock(side_effect=Exception("Stream creation failed"))
