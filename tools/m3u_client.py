...
class M3UProxyClient:
    ...
    def trigger_failover(self, stream_id):
        ...
        response = requests.post(
            f"{self.base_url}/streams/{stream_id}/failover")
        return response.json()
    ...
    def print_stats(self):
        ...
        print(
            f"Total Data Served: {self.format_bytes(proxy_stats['total_bytes_served'])}")
        ...
        if stats["streams"]:
            ...
            for stream in stats["streams"]:
                ...
                print(
                    f"  Data Served: {self.format_bytes(stream['total_bytes_served'])}")
        ...
        if stats["clients"]:
            ...
            for client in stats["clients"]:
                ...
                print(
                    f"  Stream: {client['stream_id'][:16] if client['stream_id'] else 'None'}...")
                print(f"  User Agent: {client['user_agent']}")
                print(f"  IP: {client['ip_address']}")
                print(
                    f"  Data: {self.format_bytes(client['bytes_served']) if client['bytes_served'] else '0 B'}")
...
def main():
    ...
    failover_parser = subparsers.add_parser(
        "failover", help="Trigger failover")
    ...
    try:
        if args.command == "create":
            result = client.create_stream(
                args.url, args.failover, args.user_agent)
            ...
            if result["streams"]:
                for stream in result["streams"]:
                    stream_type = "HLS" if stream['original_url'].lower().endswith(
                        '.m3u8') else "Direct"
                    print(
                        f"{stream['stream_id']}: {stream['original_url']} ({stream_type})")
            else:
